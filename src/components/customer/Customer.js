...
export default class View extends Component {
    ...
    componentDidMount() {     
       this.setState({
        account:this.props.customer.customer,
        allData:this.props.customer
       })
    }

    delete = async (accToDelete) => {

        const url = 'http://localhost:8080/customer/' + accToDelete.customeId
        try {
    ...
    isEdit = (oldData, change) => {
        ...
        if (change) {
            ...
            const { name, email, mobile, dateOfBirth,houseName,state,city,country,pin,customeId} = this.state
            const accToUpdate = {
                name, email, mobile, dateOfBirth,houseName,state,city,country,pin,customeId
            }
    ...
    addData = async (value) => {
        ...
        if (value) {
            ...
            if (this.validateData(formData)) {
                const response = await Axios.post("http://localhost:8080//partners/customer/"+id, formData)
                console.log(response);
                if(response.data.statusCode === 201){
                    this.setState({
                        account: account,
